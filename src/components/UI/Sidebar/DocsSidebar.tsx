...
export function SidebarProvider({ children }: { children: ReactNode }) {
    ...
    return (
        <SidebarContext.Provider value={{ open, setOpenSearch }}>
            <button
                className={clsx(
                    "w-full h-8 bg-dark border-b-[1px] px-8 sm:px-14 text-white",
                    "flex flex-row gap-2 items-center text-sm text-white",
                    "sticky inset-x-0 top-12 z-50 lg:hidden text-white"
                )}
...
export function SidebarList({ items }: { items: TreeNode[] }) {
    ...
    return (
        <aside
            className={clsx(
                "flex flex-col gap-3 fixed inset-0 top-20 overflow-auto text-white bg-dark",
                "lg:sticky lg:top-12 lg:py-16 lg:max-h-[calc(100vh-3rem)] text-white bg-dark",
                "max-lg:py-4 max-lg:px-8 max-lg:sm:px-14 bg-dark backdrop-blur-xl max-lg:z-50 text-white",
                !open && "max-lg:hidden"
            )}
        >
            <button
                className="flex flex-row items-center border-[1px] rounded-md cursor-pointer px-4 py-2 text-left text-sm text-white"
...
function Node({ item }: { item: TreeNode }) {
    if (item.type === "separator")
        return (
            <p className="font-semibold text-sm mt-3 first:mt-0 bg-dark">{item.name}</p>
        );
...
function Folder({ item }: { item: FolderNode }) {
    ...
    return (
        <div className="w-full bg-dark">
            ...
            <ul
                className={clsx(
                    "flex-col mt-3 transition-all overflow-hidden bg-dark",
                    extend ? "flex" : "hidden"
                )}
            >
                {children.map((item, i) => {
                    ...
                    return (
                        <li
                            key={i}
                            className={clsx(
                                "flex pl-4 py-1.5 border-l-2 border-border bg-dark",
                                active ? "border-purple-400 bg-dark" : "border-border bg-dark"
                            )}
                        >
                            <Node item={item} />
                        </li>
