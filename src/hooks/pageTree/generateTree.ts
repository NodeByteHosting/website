...
export function buildPageTree(meta: Meta): TreeNode[] {
    const folder = meta._raw.sourceFileDir.split("/").filter((c: any) => c !== ".");

    return meta.pages.flatMap<TreeNode>((item: any) => {
        const separator = /---(.*?)---/;
        ...
        const page = allDocs.find((page: any) => {
            return (
        ...
        const meta = allMeta.find((meta: any) => meta._raw.sourceFileDir === path);
